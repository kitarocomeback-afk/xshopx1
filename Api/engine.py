"""
Shopify Checkout Engine — V7
==============================
Async + curl_cffi (chrome120) + full GraphQL query.
Fixed: AsyncSession has no .closed attribute.

Key Features:
- AsyncSession (curl_cffi) — TLS fingerprint chrome120
...
class ShopifyEngine:
    ...
    async def _get_session(self) -> AsyncSession:
        # ⭐ FIX: don't check .closed — AsyncSession has no such attribute
...
async def _run_checkout_async(shop_url: str, card_entry: str, proxy_url: str = "") -> CheckResult:
    ...
    if status == "charged":
        ...
        if "CAPTCHA" in reason:
            ...
            result.retryable = True        result.error = Exception(reason)
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    ...
    Always creates a new event loop via asyncio.run().
    """
