...
Public API:
    run_checkout_for_card(shop_url, card_entry, proxy_url, low=True) -> CheckResult
    normalize_proxy(raw) -> str
    parse_card_entry(entry) -> (number, month, year, cvv)

Flow:
    Step 0  — Find cheapest product (max_price)
    Step 1  — Add to cart, start checkout
    Step 2  — Private access token
    Step 3  — Actions JS, extract operation IDs
    Step 4  — Proposal 1 (currency/country)
    Step 5  — Proposal 2 (email)
    Step 6  — Proposal 3 (address + fallback)
    Step 7  — Proposal 4 (confirm)
    Step 8  — Proposal 5 (PendingTerms wait)
    Step 9  — PCI tokenization
    Step 10 — SubmitForCompletion (tax retry)
    Step 11 — PollForReceipt

Anti-CAPTCHA Strategy:
    - Browser profile rotation (chrome131, chrome124, ...)
    - Proxy rotation per request
    - Address database (28 countries + fallback)
    - Random email/phone per attempt
    - CAPTCHA detection & retry with new identity
"""
...
# Latest browser profiles first (chrome131 is current)
BROWSER_PROFILES = [
    "chrome131",
    "chrome124",
    "chrome123",
...
def find_cheapest_product(client: TLSClient, shop_url: str,
    ...
    if in_range:
        ...
        # all products are over max price
        cheapest = fallback[0]
...
def send_pci_session(ident_sig: str, card_number: str, card_name: str,
                     ...
                     impersonate: str = "chrome131") -> Tuple[int, str]:
...
def _build_proposal_payload(stable_id, variant_id, session_token, proposal_id,
    ...
    """Build Proposal GraphQL payload (used by Step 4-8)."""
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    ...
    # Random identity per attempt (anti-CAPTCHA)
