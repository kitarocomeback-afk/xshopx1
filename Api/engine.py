"""
Shopify Checkout Engine — NEWSS-STYLE V6
==========================================
Async + curl_cffi (chrome120) + full GraphQL query + static queries.
No SUBMIT_FAILED — uses /checkouts/unstable/graphql.

Key Features:
- AsyncSession (curl_cffi) — TLS fingerprint
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    """
    Sync wrapper — called by api_server.py.
    Internally runs asyncio event loop for async engine.
    """
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # FastAPI is async — use new loop in thread
            import concurrent.futures
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                future = pool.submit(asyncio.run, _run_checkout_async(shop_url, card_entry, proxy_url))
                return future.result(timeout=REQUEST_TIMEOUT + 30)
        else:
            return loop.run_until_complete(_run_checkout_async(shop_url, card_entry, proxy_url))
    except RuntimeError:
